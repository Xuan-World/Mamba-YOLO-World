...
class TextGuidedSS2D2_noGuide(SS2D_mamba2):
    ...
    def forward_mm(self, x: torch.Tensor, **kwargs):
        x = self.in_proj(x)
        ...
        x = self.act(x)
        y,final_state= self.forward_core(x)
        y = self.out_act(y)
...
@MODELS.register_module()
class TextGuidedODSSBlock2_noGuide(XSSBlock):
...
@MODELS.register_module()
class MambaFusionCSPLayerWithTwoConv2_noGuide(CSPLayerWithTwoConv):
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
    ...
    def forward(self, x: Tensor) -> tuple[Any, Any]:
        ...
        text_guided_x, x_hidden_state =  self.textGuidedVSSBlock(x_main[-1])
